...
class PyTorchGMRModel(nn.Module):
    ...
    def __call__(self, inputs, test_cases, parameters, validate_args=False):
        ...
        energy = W_dev + W_vol
        return energy.unsqueeze(-1) # [num_points, 1] to support 3D array transposition during sensitivity analysis
...
def main():
    ...
    if not os.path.exists(export_dir) or not os.path.exists(os.path.join(export_dir, "mean_psi.npy")):
        import subprocess
        print(f"'{export_dir}' not found. Exporting GP to PyTorch first (sample_mode: {args.sample_mode}, max_gamma: {args.max_gamma})...")
        subprocess.run(["python3", "distillation/export_gp_to_pytorch.py", "--saved_model_dir", args.saved_model_dir, "--sample_mode", args.sample_mode, "--num_points", str(args.num_points), "--max_gamma", str(args.max_gamma), "--export_subfolder", export_subfolder], check=True)
    ...
    model = PyTorchGMRModel(num_points, device=device)
    output_selector = EnergyOutputSelector(num_outputs=num_points)
    ...
    if args.load_distilled_dir and os.path.exists(args.load_distilled_dir):
        ...
        out_dir = os.path.abspath(os.path.join("distillation", "distilled_models", f"{current_time}_{ugp_model_name}_{args.material_model}{mode_str}_uqmodeldisc"))
        log_mode = "w"
