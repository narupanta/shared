...
if __name__ == "__main__" :
    ...
    asym_factor = 0.0
    loads_top = jnp.linspace(1.0, 7, 10)
    ...
    u_pred_mean = jnp.array(u_pred_samples)[:, :, :].mean(axis=0)
    u_pred_std = jnp.array(u_pred_samples)[:, :, :].std(axis=0)

    plot_disp_field(node_coords, cells, u_true, u_pred_mean, u_pred_std, save_path)

    f_true2x2, _ = deformation_gradient_element(node_coords[cells], u_true[cells])
