...
def main():
    ...
    print("Precomputation finished! Launching Web UI...")
    
    def plot_gamma(val):
        idx = np.argmin(np.abs(gamma_vals - val))
        g_val = gamma_vals[idx]
        
        fig, axes = plt.subplots(2, 3, figsize=(15, 9))
        fig.suptitle(f"Energy Distribution Cross-Section at $\\gamma={g_val:.2f}$ (Distilled: {args.material_model.upper()})", fontsize=18)
        axes = axes.flatten()
    ...
    with gr.Blocks(title="Distilled WGAN-GP Interactive Physics") as demo:
        gr.Markdown("# 🚀 Interactive Energy Distribution Cross-Section")
        gr.Markdown("Slide the stretch parameter $\gamma$ to watch how the WGAN-GP Normalizing Flow tracks the Bayesian GP uncertainty bounds!")
        
        gamma_slider = gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.02, label="Gamma (Stretch)")
        plot_output = gr.Plot()
        
        # Initial Plot
        demo.load(fn=plot_gamma, inputs=[gamma_slider], outputs=[plot_output])
        # Update on slider change
        gamma_slider.change(fn=plot_gamma, inputs=[gamma_slider], outputs=[plot_output])

    # Launch with share=True to bypass Docker port mapping issues!
